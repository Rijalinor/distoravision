@extends('layouts.app')
@section('page-title', 'Outlet Growth Trajectory (Trend 6 Bulan)')
...
<div class="kpi-grid" style="grid-template-columns: repeat(5, 1fr);">
    <a href="{{ route('analytics.outlet-trajectory', ['segment' => 'Growing', 'period' => $period, 'principal_id' => request('principal_id', 'all')]) }}" 
    ...
    <a href="{{ route('analytics.outlet-trajectory', ['segment' => 'Stable', 'period' => $period, 'principal_id' => request('principal_id', 'all')]) }}" 
    ...
    <a href="{{ route('analytics.outlet-trajectory', ['segment' => 'Declining', 'period' => $period, 'principal_id' => request('principal_id', 'all')]) }}" 
    ...
    <a href="{{ route('analytics.outlet-trajectory', ['segment' => 'New', 'period' => $period, 'principal_id' => request('principal_id', 'all')]) }}" 
       class="card kpi-card" style="text-decoration:none; border-top: 4px solid var(--primary-light); {{ $segment === 'New' ? 'background:rgba(99,102,241,0.1);' : '' }}">
        ...
        <div class="kpi-label text-center">Toko Baru</div>
    </a>

    <a href="{{ route('analytics.outlet-trajectory', ['segment' => 'Dead', 'period' => $period, 'principal_id' => request('principal_id', 'all')]) }}" 
       class="card kpi-card" style="text-decoration:none; border-top: 4px solid var(--accent-red); {{ $segment === 'Dead' ? 'background:rgba(239,68,68,0.1);' : '' }}">
        ...
        <div class="kpi-label text-center">Mati / Berhenti</div>
    </a>
</div>
    ...
    <div class="card-header" style="display:flex; justify-content:space-between; align-items:center;">
        <span class="card-title">
            ...
            @if($segment !== 'all')
                ...
                <a href="{{ route('analytics.outlet-trajectory', ['period' => $period, 'principal_id' => request('principal_id', 'all')]) }}" style="font-size:0.75rem; margin-left:0.5rem; color:var(--text-muted); text-decoration:underline;">Clear Filter</a>
            @endif
        </span>
    </div>
    ...
    <div style="overflow-x:auto;">
        <table class="data-table">
            ...
            <tbody>
                ...
                <tr>
                    <td>
                        <div style="font-weight:600;">{{ Str::limit($t->outlet_name, 30) }}</div>
                        <div style="font-size:0.7rem;color:var(--text-muted);">
                            {{ $t->outlet_code }} | {{ $t->city }} | Aktif {{ $t->active_months }} bln
                        </div>
                    </td>
