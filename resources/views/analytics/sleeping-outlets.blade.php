...
<div class="alert alert-error" style="background: rgba(239, 68, 68, 0.15); border-color: rgba(239, 68, 68, 0.4);">
    ...
    <div>
        ...
        Toko-toko berikut tercatat memiliki transaksi pada periode <strong>{{ \Carbon\Carbon::parse($previousPeriod.'-01')->translatedFormat('F Y') }}</strong>, namun <u>sama sekali tidak melakukan transaksi (Faktur = 0)</u> pada periode <strong>{{ \Carbon\Carbon::parse($period.'-01')->translatedFormat('F Y') }}</strong>.
    </div>
</div>
        ...
        <table class="data-table">
            <thead>
                <tr>
                    ...
                    <th class="text-right">Sales Terakhir ({{ \Carbon\Carbon::parse($previousPeriod.'-01')->format('M Y') }})</th>
                </tr>
            </thead>
            <tbody>
